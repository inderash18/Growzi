...
import { GraduationCap, Lock, Mail, ArrowRight, Sparkles, CheckCircle2, ArrowLeft } from "lucide-react";
...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (matched) {
        setUser({
          id: matched.email === "admin@apex.edu" ? "usr-admin" : "usr-student",
          email: matched.email,
          name: matched.name,
          role: matched.role,
        });
        ...
        setLoading(false);
        toast(`Signed in successfully as ${matched.name}`, "success");
      ...
      if (email && password) {
        const role = email.includes("admin") ? "ADMIN" as const : "STUDENT" as const;
        const mockUser = {
          id: "usr-dynamic",
          email: email,
          name: email.split("@")[0],
  ...
  const handleQuickLogin = (emailStr: string) => {
    ...
    toast(`Preset selected. Click 'Sign In' to enter.`, "info");
  };

  const handleGoogleLogin = () => {
    toast("Google authentication is in demo mode. Signing in as test student.", "info");
    const mockUser = {
      id: "usr-google",
  ...
  return (
    <div className="min-h-screen text-zinc-100 flex items-center justify-center relative font-sans overflow-hidden bg-zinc-950">
      ...
      <div className="w-full max-w-[440px] px-6 py-8 relative z-10 animate-fade-in">
        <div className="bg-zinc-900/60 border border-white/10 backdrop-blur-xl shadow-2xl rounded-2xl p-8 space-y-6">
          ...
          <div className="text-center space-y-2">
            ...
            <p className="text-xs text-zinc-400 font-normal">
              Enter credentials below to enter your workspace catalog.
            </p>
          </div>
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <div className="space-y-1.5">
              <div className="flex justify-between items-center select-none">
                <Label className="text-zinc-400 mb-0">Password</Label>
                <Link 
                  href="/forgot-password" 
                  className="text-[10px] text-zinc-400 hover:text-zinc-200 transition-colors font-semibold"
                >
                  Forgot Password?
                </Link>
              </div>
            ...
            <div className="flex items-center justify-between py-1 select-none">
              <label className="flex items-center gap-2 cursor-pointer group">
                <input 
                  ...
                  className="rounded border-white/10 bg-zinc-950/40 text-blue-600 focus:ring-blue-500/10 focus:ring-offset-0 h-4 w-4 transition-all"
                />
                <span className="text-[11px] text-zinc-400 group-hover:text-zinc-350 transition-colors font-medium">Remember my session</span>
              </label>
            </div>
            ...
            <Button
              ...
              className="w-full font-bold select-none cursor-pointer bg-blue-600 hover:bg-blue-700 border-none h-10 rounded-xl"
            >
              Sign In
            </Button>
          </form>
          ...
          <div className="relative flex py-1 items-center select-none">
            ...
            <span className="flex-shrink mx-4 text-[9px] uppercase tracking-wider text-zinc-500 font-bold">Or continue with</span>
            <div className="flex-grow border-t border-white/5"></div>
          </div>
          ...
          <p className="text-xs text-center text-zinc-400 select-none">
            New to Growzi?{" "}
            <Link href="/signup" className="text-blue-400 hover:text-blue-300 transition-colors font-bold">
              Sign up
            </Link>
          </p>

          {/* Quick login presets */}
          <div className="pt-5 border-t border-white/5 select-none space-y-2.5">
            <span className="block text-[9px] font-bold uppercase tracking-widest text-zinc-500 text-center">
              Demo Test Accounts
            </span>
            <div className="grid grid-cols-2 gap-3 text-xs">
              <button
                ...
                className="p-3 bg-zinc-950/20 hover:bg-zinc-950/40 border border-white/5 hover:border-white/10 rounded-xl text-left transition-all group flex items-center justify-between cursor-pointer"
              >
                <div>
                  <span className="font-bold block text-zinc-200 text-[10px] truncate">Alex Carter</span>
                  <span className="text-zinc-500 block text-[8px] mt-0.5">Student Role</span>
                </div>
                <ArrowRight className="h-3.5 w-3.5 text-zinc-600 group-hover:text-blue-400 group-hover:translate-x-0.5 transition-all shrink-0 ml-1" />
              </button>
              ...
              >
                <div>
                  <span className="font-bold block text-zinc-200 text-[10px] truncate">Marcus Vance</span>
                  <span className="text-zinc-500 block text-[8px] mt-0.5">Admin Role</span>
                </div>
                <ArrowRight className="h-3.5 w-3.5 text-zinc-600 group-hover:text-blue-400 group-hover:translate-x-0.5 transition-all shrink-0 ml-1" />
              </button>
            </div>
